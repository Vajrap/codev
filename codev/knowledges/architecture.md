...
5. **Initial Agent Instructions**: The scaffolded `AGENTS.md` already includes explicit persistence rules for durable knowledge, decisions, ticket context, and session notes before post-init rewrite.
...
3. **Folder Scan**: `/api/scan` reads the requested folder under the served project root and returns child folders plus Markdown files.
...
6. **Dashboard UI**: Static assets under `dashboard/` render the project browser, Markdown preview, editor, and save flow.
